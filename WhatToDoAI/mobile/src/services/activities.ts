...
import { API_KEYS, APP_CONSTANTS, API_TIMEOUT } from '../config'; // Correct import
import { Activity, AISummary } from '../types/activity';
 ...
 */
export const fetchEventbriteActivities = async (params: SearchParams): Promise<Activity[] | null> => {
    const {
        location,
    ...
    if (startDate) apiParams['start_date.range_start'] = startDate.toISOString().slice(0, 19) + 'Z';
    if (endDate) {
        apiParams['start_date.range_end'] = endDate.toISOString().slice(0, 19) + 'Z';
    } else {
    ...
    try {
        ...
        const response = await axios.get(EVENTBRITE_API_URL, {
            ...
            timeout: API_TIMEOUT, // Correct usage
        });
        ...
        if (response.data && response.data.events) {
            const activities: Activity[] = response.data.events.map((event: any) => ({
                ...
                start_time: event.start?.utc ? new Date(event.start.utc) : undefined,
                end_time: event.end?.utc ? new Date(event.end.utc) : undefined,
 ...
 */
export const fetchTripAdvisorActivities = async (params: SearchParams): Promise<Activity[] | null> => {
    const {
        location,
        radius = APP_CONSTANTS.DEFAULT_RADIUS,
        query,
    ...
    try {
        ...
        const response = await axios.get(TRIPADVISOR_API_URL, {
            params: apiParams,
            headers: {
                'X-RapidAPI-Key': API_KEYS.TRIPADVISOR_API_KEY || API_KEYS.RAPIDAPI_KEY,
 ...
 */
export const searchActivities = async (params: SearchParams): Promise<Activity[] | null> => {
    try {
...
export const getActivityDetails = async (activityId: string, source: 'Eventbrite' | 'TripAdvisor'): Promise<Activity | null> => {
    ...
    try {
        ...
        if (activity) {
            const summary = await generateAISummary(activity);
            activity.aiSummary = summary || undefined; // Convert null to undefined if needed
        }
