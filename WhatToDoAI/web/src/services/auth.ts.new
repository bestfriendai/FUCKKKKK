// WhatToDoAI/web/src/services/auth.ts

import { supabase } from './supabaseClient';
import { SignInCredentials, SignUpCredentials } from '../types/auth'; // Assuming types will be defined here
...
export const signUp = async ({ email, password, username }: SignUpCredentials) => {
  const { data, error } = await supabase.auth.signUp({
      ...
      options: {
          ...
          // For web, you might want to specify an email redirect URL for confirmation
...
export const sendPasswordResetEmail = async (email: string) => {
      const { data, error } = await supabase.auth.resetPasswordForEmail(email, {
              // Specify the redirect URL for the password reset confirmation
